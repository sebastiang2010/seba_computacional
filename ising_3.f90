...
!!!!!
!!! alocate 
allocate(S(0:nising+1,0:nising+1), Mn(niter),En(niter),aceptado(niter))
!!!!!!!
...
write(2,*) "#   n // T[K] // f // c // M // E"
do n=1,niter
        !! lo corro para que no de el 1,1 
        !! cuando lo ponga en forma de toroide usar 1       
        f=nint(uni()*(nising-1))+1 !init redondea 
        c=nint(uni()*(nising-1))+1
        ...
        do f=1,nising
          ...
          !if (n==1) then 
              do f1=1,nising
                 do c1=1,nising
                !! es el Hamiltoniano
                E=E-J*S(f1,c1)*(S(f1+1,c1)+S(f1,c1+1)+S(f1-1,c1)+S(f1,c1-1))-Ho*S(f1,c1)      
                M=M+S(f1,c1)
                end do 
              end do         
          En(n)=E
         ...
         write(2,fmt="(i7,x,f10.3,x,i5,x,i5,x,x,i5,x,x,i5,x,x,i7,x,x,f10.3,x,x,f10.3)") n, T,f,c,aceptado(n), Mn(n), En(n),p,dE
end do 
close(2)
